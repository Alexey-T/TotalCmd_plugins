...
type
  PWAVPackfile = ^TWAVPackfile;
  TWAVPackfile =  object(TObj)
  private
   	FFileSize: int64;
		FValid: boolean;
		FFormatTag: integer;
  ...
  public
    ...
    function _ReadV3( f: PStream ): boolean;
    function _ReadV4( f: PStream ): boolean;
    ...
    property APEtag: PAPE read FAPEtag;
    property ID3v1: PID3v1 read FID3v1tag;
	end;

function NewWAVPack: PWAVPackfile;
...
function TWAVPackfile.ReadFromFile(const FileName: WideString): Boolean;
var
  f: PStream;
...
begin
  ...
  FResetData;
  FID3v1tag.ReadFromFile(FileName);
  FTagSize := FID3v1tag.Size;
  FAPEtag.ReadFromFile(FileName);
  FTagSize := FTagSize + FAPEtag.Size;
...
function TWAVPackfile._ReadV4( f: PStream ): boolean;
var
...

{
    ...
    begin
      ...
      begin
        ...
        else if encoderbyte = 24 then
            FEncoder := FEncoder + ' (extra high)'
        else if encoderbyte = 96 then
            FEncoder := FEncoder + ' (normal)'
        else if encoderbyte = 98 then
            FEncoder := FEncoder + ' (fast)'
        else if encoderbyte = 104 then
            FEncoder := FEncoder + ' (high)'
        else if encoderbyte = 120 then
            FEncoder := FEncoder + ' (extra high)';
        Break;
      end;
    end;
...
function TWAVPackfile._ReadV3( f: PStream ): boolean;
var
  chunk: riff_chunk;
...
begin
  ...
  while (f.Position < f.Size) do begin
     ...
     f.seek( fpos + chunk.size, spCurrent );
  end; // while
...
begin
  ...
  if FValid then
    Result := (FFileSize - FTagSize) / (FSamples * (FChannels * FBits / 8) + 44) * 100
  else
    Result := 0;
end;
