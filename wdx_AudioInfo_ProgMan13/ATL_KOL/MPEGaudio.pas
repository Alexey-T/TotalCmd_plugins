...
uses Windows, KOL, ID3v1, ID3v2, APE;
...
type
  ...
  function NewMPEGaudio: PMPEGaudio;
...
const
  { Table for bit rates }
  _MPEG_BIT_RATE: array [0..3, 0..3, 0..15] of Word =
    (
    { For MPEG 2.5 }
...
begin
  { Get bit rate }
  Result := _MPEG_BIT_RATE[Frame.VersionID, Frame.LayerID, Frame.BitRateID];
end;
...
const
  ...
  _MPEG_SAMPLE_RATE: array [0..3, 0..3] of Word =
    (
...
begin
  { Get sample rate }
  Result := _MPEG_SAMPLE_RATE[Frame.VersionID, Frame.SampleRateID];
end;
...
begin
  { Search for vendor ID }
  Result := '';
  if (SizeOf(Data) - Size) < 0 then Size := SizeOf(Data);
  Size := SizeOf(Data) - 8;
  for Iterator := 0 to Size - 4 do
  begin
    Finalize(VendorID);
    VendorID :=
      Chr(Data[Iterator]) +
    ...
    begin
      Result := VendorID;
        for J := Iterator + 4 to Size do
            if (Data[J] <> $AA) and
              (Data[J] <> $55) and
              ((Data[J] > 31) and (Data[J] < 129)) then
            Result := Result + Chr(Data[J])
          else
            Break;
      if Trim(Result) = 'LAME' then
        Continue;
      break;
    end;
    if VendorID = VENDOR_ID_GOGO_NEW then
...
const
  { MPEG version names }
  _MPEG_VERSION: array [0..3] of string =
    ('MPEG 2.5', 'MPEG ?', 'MPEG 2', 'MPEG 1');
begin
  { Get MPEG version name }
  Result := _MPEG_VERSION[FFrame.VersionID];
end;
...
const
  { MPEG layer names }
  _MPEG_LAYER: array [0..3] of string =
    ('Layer ?', 'Layer III', 'Layer II', 'Layer I');
begin
  { Get MPEG layer name }
  Result := _MPEG_LAYER[FFrame.LayerID];
end;
...
const
  { Channel mode names }
  _MPEG_CM_MODE: array [0..4] of string =
    ('Stereo', 'Joint Stereo', 'Dual Channel', 'Mono', 'Unknown');
begin
  { Get channel mode name }
  Result := _MPEG_CM_MODE[FFrame.ModeID];
end;
...
const
  { Emphasis names }
  _MPEG_EMPHASIS: array [0..3] of string =
    ('None', '50/15 ms', 'Unknown', 'CCIT J.17');
begin
  { Get emphasis name }
  Result := _MPEG_EMPHASIS[FFrame.EmphasisID];
end;
...
begin
  ...
  begin
    if FID3v1.Exists then MPEGSize := FFileLength - FID3v2.Size - FID3v1.Size{128}
    else MPEGSize := FFileLength - FID3v2.Size;
    if FAPE.Exists then
      MPEGSize := MPEGSize - FAPE.Size;
...
const
  { Encoder names }
  _MPEG_ENCODER: array [0..7] of string =
    ('Unknown', 'Xing', 'FhG', 'LAME', 'Blade', 'GoGo', 'Shine', 'QDesign');
var
  VendorID: string;
begin
  { Get guessed encoder name and encoder version for LAME }
  Result := _MPEG_ENCODER[FGetEncoderID];
  if FVBR.VendorID <> '' then VendorID := FVBR.VendorID;
  if FVendorID <> '' then VendorID := FVendorID;

  if (FGetEncoderID = MPEG_ENCODER_LAME) then
    Result := Trim(Result + #32 + Trim(Copy(FVendorID, 5, MaxInt)));

  {
  if (FGetEncoderID = MPEG_ENCODER_LAME) and (Result = 'LAME') and
    ...
    begin
      Result :=
        ...
        VendorID[7] +
        VendorID[8];
      if (Length(VendorID) >= 9) and
        (VendorID[9] in ['0'..'9']) then
        Result := Result + VendorID[9];
    end;
  }
end;
...
procedure TMPEGaudio.InitFields;
begin
  FResetData;
end;
...
begin
  ...
  if (FID3v2.ReadFromFile(FileName)) and (FID3v1.ReadFromFile(FileName) and (FAPE.ReadFromFile(FileName))) then
    try
...
FVendorID := FindVendorID(Data, FFrame.Size * 5);
        ...
        begin
          SourceFile.Seek((FFileLength - FID3v2.Size) div 2, spBegin);
          SourceFile.Read(Data, SizeOf(Data));
          FFrame := FindFrame(Data, FVBR);
        end;
{
  if (FVendorID = '') or (FVendorID = 'LAME') then
    FVendorID := FindVendorID(Data, FFrame.Size * 5);
end else if (Transferred = SizeOf(Data)) and ((FVendorID = '') or (FVendorID = 'LAME')) then
begin
  SourceFile.Seek(Trunc((FFileLength - FID3v2.Size) / 1.75), spBegin);
  SourceFile.Read(Data, SizeOf(Data));
  FVendorID := FindVendorID(Data, FFrame.Size * 5);
end;
}
...
if (FFrame.Found) and (FVendorID = '') then
        begin
          if not FID3v1.Exists then SourceFile.Seek(FFileLength - SizeOf(Data), spBegin)
          else SourceFile.Seek(FFileLength - SizeOf(Data) - FID3v1.Size{128}, spBegin);
          SourceFile.Read(Data, SizeOf(Data));
          FVendorID := FindVendorID(Data, FFrame.Size * 5);
        end;
        Result := true;
