...
type
  ...
  private
    ...
    function ReadHeader(const FileName: string; var ATag: TTagInfo): Boolean; overload;
    function ReadHeader(const Stream: PStream; var ATag: TTagInfo): Boolean; overload;
    procedure ReadFrames(const Stream: PStream; var ATag: TTagInfo;
      IsNew: Boolean; SeekFromCurrent: Boolean = False);
    ...
    function GetFrameTextEx(Frame: PFrame): String;
  ...
  public
    FVersionID: Byte;
    UseExtendedGetFrameText, FixTag24Sync, CalcAllID3SizesInFile: Boolean;

    constructor Create;
    destructor Destroy; virtual;
    procedure ResetData;

    function ReadFromFile(const FileName: string; FromPos: Int64 = 0): Boolean;                     // Load tag
    function ReadFromStream(const Stream: PStream): Boolean;                    // Load tag
    function ReadFromStreamEx(const Stream: PStream): Boolean;                  // Load tag
...
function TID3v2.ReadFromFile(const FileName: string; FromPos: Int64 = 0): Boolean;
var _Tag: TTagInfo;
  SourceFile: PStream;
  ASize, TempSize: Integer;
begin
  try
    ...
    try
      FillChar(_Tag, SizeOf(Tag), 0);
      if FromPos > 0 then
        SourceFile.Position := FromPos;
      Result := ReadHeader(SourceFile, _Tag);
      ...
      if (Result) and (_Tag.ID = ID3V2_ID) then begin
        ...
        begin
          ReadFrames(SourceFile, _Tag, FVersionID > TAG_VERSION_2_2);

          if CalcAllID3SizesInFile then
          begin
            repeat
              SourceFile.Seek(ASize, spBegin);
              if SourceFile.Position >= Sourcefile.Size then
                Break;
              FillChar(_Tag, SizeOf(Tag), 0);
              if ReadHeader(SourceFile, _Tag) and (_Tag.ID = ID3V2_ID) then
              begin
                TempSize := GetTagSize(_Tag);
                if TempSize <= 0 then
                  Break;
                ASize := ASize + TempSize;
              end else
                Break;
...
function TID3v2.ReadFromStream(const Stream: PStream): Boolean;
var _Tag: TTagInfo;
begin
  try
    { Reset data and load header from file to variable }
    ResetData;
      Result := ReadHeader(Stream, _Tag);
      { Process data if loaded and header valid }
      if (Result) and (_Tag.ID = ID3V2_ID) then begin
        FExists := true;
        { Fill properties with header data }
        ...
        if (FVersionID in [TAG_VERSION_2_2..TAG_VERSION_2_4]) and (FSize > 0) then begin
          ReadFrames(Stream, _Tag, FVersionID > TAG_VERSION_2_2);
        end;
      end;
  except
    Result := False;
...
function TID3v2.ReadFromStreamEx(const Stream: PStream): Boolean;
var _Tag: TTagInfo;
  SavedPosition: Int64;
begin
  try
    ...
    SavedPosition := Stream.Position;
      ...
      if (Result) and (_Tag.ID = ID3V2_ID) then begin
        ...
        FSize := GetTagSize(_Tag);
        { Get information from frames if version supported }
    Stream.Seek(SavedPosition, spBegin);
        if (FVersionID in [TAG_VERSION_2_2..TAG_VERSION_2_4]) and (FSize > 0) then begin
          ReadFrames(Stream, _Tag, FVersionID > TAG_VERSION_2_2, True);
        end;
      end;
  except
    Result := False;
...
procedure TID3v2.ReadFrames(const Stream: PStream; var ATag: TTagInfo;
  IsNew: Boolean; SeekFromCurrent: Boolean = False);
const FrameSize : array [Boolean] of Integer = (6, 10);
var Frame: TFrameHeader;
    Data: Pointer;
    ...
    begin
      if SeekFromCurrent then
        Result := ((Stream.Size - Stream.Position) < TagSize) and (Stream.Position < Stream.Size)
      else
        Result := (Stream.Position < TagSize) and (Stream.Position < Stream.Size);
    end;
...
begin
    try
        if SeekFromCurrent then
          ...
          Stream.Seek(10, spBegin);

        TagSize:= GetTagSize(ATag);
        while CheckStep {(Stream.Position < TagSize) and (Stream.Position < Stream.Size)} do begin
          Stream.Read(Frame, FrameSize[IsNew]);
          ...
          if SeekFromCurrent then begin
            if DataSize > (Stream.Size - DataPosition) then Break
          end else
...
//            ID[4]:= ID[3];
          ...
          Stream.Seek(DataPosition + DataSize, spBegin);
        end;
    except
    end;
end;
...
function TID3v2.ReadHeader(const Stream: PStream; var ATag: TTagInfo): Boolean;
var
  Transferred: Integer;
begin
      Result := true;
      try
        { Read header and get file size }
        Transferred:= Stream.Read(ATag, 10);
        ATag.FileSize := Stream.Size;
        { if transfer is not complete }
        if Transferred < 10 then Result := false;
      except
        Result := false;
...
begin
  inherited;
  UseExtendedGetFrameText := False;
  FixTag24Sync := False;
  CalcAllID3SizesInFile := False;
end;
...
begin
    Result:= nil;
    ATag := UpperCase(Index);
    for i:= 0 to FTagList.Count-1 do
      if UpperCase(Tags[i].ID) = ATag then begin
        Result:= Tags[i];
        Break;
      end;
end;
...
function TID3v2.GetFrameTextEx(Frame: PFrame): String;
label 1;
var
    WC, OldWC, SecondWC: PAnsiChar;
    Temp: AnsiString;
    Len, OldLen, SecondLen, Tag24SyncSpecial: Integer;
    S: AnsiString;
...
1:
        ...
        IsUnicode   := (Byte(WC^) = 1);
        IsUnicodeBE := (Byte(WC^) = 2);
        IsUTF8      := (Byte(WC^) = 3);
        if IsUnicode or IsUnicodeBE or IsUTF8 then
        begin
          Inc(WC);
          Dec(Len);
        end;

        if (Frame.ID = ID3V2_Frames[tiLink].New) and (not Flag) then
        begin
          OldLen := Len;
          OldWC := WC;
          if IsUnicode or IsUnicodeBE then
          begin
            while (WideChar(WC^) <> #0)and(Len > 1) do begin inc(WC, 2); dec(Len, 2); end;
          end else
        ...
        if FixTag24Sync and (not Flag) then
          if ((FVersionID = TAG_VERSION_2_4) and ((Frame.Flags and $0200) = $0200) and (not (IsUnicode or IsUnicodeBE))) then
          begin
            if Len > 8 then
            begin
              Tag24SyncSpecial := 5;
              ...
              if Byte(WC^) = $FF then
              begin
                Tag24SyncSpecial := Tag24SyncSpecial + 2;
                Inc(WC, 2);
                Dec(Len, 2);
                if (Byte(WC^) = $FE) or (Byte(WC^) = 0) then
                begin
                  Byte(WC^) := 0; // Change data!
                  IsUnicodeBE := True;
                  Dec(WC, 3);
                  Inc(Len, 3);
                  Tag24SyncSpecial := -1;
              ...
              if Tag24SyncSpecial > -1 then
              begin
                Dec(WC, Tag24SyncSpecial);
                Inc(Len, Tag24SyncSpecial);
              end;
            end;
          end;
...
function TID3v2.GetHasCoverArt: Boolean;
begin
  Result := Assigned(GetTagsByName('ApiC'));
end;
...
begin
    if UseExtendedGetFrameText then
    begin
      Result := GetFrameTextEx(Frame);
      Exit();
    end;
...
begin
    Result:=0;
    for i:= 0 to FTagList.Count-1 do
      if UpperCase(Tags[i].ID) = 'COMM' then Inc(Result);
end;
...
begin
    ...
    for i:= 0 to FTagList.Count-1 do
    begin
      if UpperCase(Tags[i].ID) = 'COMM' then
      begin
        Inc(Cnt);
...
begin
    ...
    if not UseExtendedGetFrameText then
    repeat
      i:= Pos(#0, Result);
