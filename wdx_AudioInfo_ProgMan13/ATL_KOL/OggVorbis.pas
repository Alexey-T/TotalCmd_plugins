...
type
  ...
  { Class TOggVorbis }
  POggVorbis = ^TOggVorbis;
  TOggVorbis = object(TObj)
    private
    ...
    public
      ...
      property EncoderTag: string read FEncoderTag;
      property HasCoverArt: Boolean read FHasCoverArt;
  end;

function NewOggVorbis: POggVorbis;
...
function GetID3v2Size(const Source: PStream): Integer;
type
  ID3v2Header = record
...
begin
  ...
  begin
    ...
    if FieldID = 'METADATA_BLOCK_PICTURE' then
...
procedure ReadTag(const Source: PStream; var Info: FileInfo);
var
...
begin
  ...
  repeat
    ...
    Source.Seek(Position + Size, spBegin);
...
function GetSamples(const Source: PStream): Integer;
var
...
begin
  ...
  Result := 0;
  for Index := 0 to 10 * 4096 do
  begin
    DataIndex := Source.Size - (SizeOf(Data) - 10) * Index - 10;
    Source.Seek(DataIndex, spBegin);
    if (Source.Position > Source.Size) or (Source.Position < 0) then
      Break;
    ...
    for Iterator := SizeOf(Data) - 10 downto 0 do
      ...
      begin
        Source.Seek(DataIndex + Iterator, spBegin);
        Source.Read(Header, SizeOf(Header));
...
begin
  ...
  try
    ...
    SourceFile.Seek(Info.SPagePos + Info.SPage.Segments + 27, spBegin);
...
function BuildTag(const Info: FileInfo): PStream;
var
  Index, Fields, Size: Integer;
  FieldData: string;
  ID: array [1..7] of AnsiChar;
begin
  { Build Vorbis tag }
...
procedure SetCRC(const Destination: PStream; Info: FileInfo);
var
  Index: Integer;
  Value: Cardinal;
...
begin
  ...
  Destination.Seek(Info.SPagePos + Info.SPage.Segments + 27, spBegin);
  for Index := 1 to Info.SPage.Segments do
  ...
  Destination.Seek(Info.SPagePos + 22, spBegin);
  Destination.Write(Value, SizeOf(Value));
end;

{ --------------------------------------------------------------------------- }

function RebuildFile(FileName: WideString; Tag: PStream; Info: FileInfo): Boolean;
var
...
begin
  ...
  Result := false;
  if (not WFileExists(FileName)) or (FileSetAttr(FileName, 0) <> 0) then exit;
  try
    { Create file streams }
    BufferName := FileName + '~';
    Source := NewReadFileStreamW(FileName);
    Destination := NewFileStream(BufferName, ofCreateAlways or ofOpenWrite or ofOpenRead or ofShareExclusive);
    { Copy data blocks }
    Stream2Stream(Destination, Source, Info.SPagePos);
    Destination.Write(Info.SPage, Info.SPage.Segments + 27);
    Tag.Position:= 0; Stream2Stream(Destination, Tag, Tag.Size);
    Source.Seek(Info.TagEndPos, spBegin);
    Stream2Stream(Destination, Source, Source.Size - Info.TagEndPos);
    SetCRC(Destination, Info);
    Source.Free;
    Destination.Free;
    { Replace old file and delete temporary file }
    if (DeleteFileW(PWideChar(FileName))) and (MoveFileW(PWideChar(BufferName), PWideChar(FileName))) then
      Result:=true
    else
      Result:=false;
    if FileExists(BufferName) then DeleteFileW(PWideChar(BufferName));
  except
    { Access error }
  end;
end;
...
begin
  ...
  if CodecType = octOpus then
  begin
     if FSamples > 0 then
         Result := FSamples  / 48000;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    FEncoderTag := Info.Tag.FieldData[10];
    FHasCoverArt := Info.HasCoverArt;
    Result := true;
  end;
  Finalize(Info);
end;
...
begin
  ...
  begin
    ...
    SetLacingValues(Info, _Tag.Size);
    Result := RebuildFile(FileName, _Tag, Info);
    _Tag.Free;
  end;
  Finalize(Info);
end;
