...
const
  ...
  TWIN_CHUNK: array [TChunkType] of AnsiString =
    ('NAME', 'COMT', 'AUTH', '(c) ', 'FILE', 'ALBM');

type
  { TwinVQ chunk header }
  ChunkHeader = record
    ID: array [1..4] of AnsiChar;                                      { Chunk ID }
  ...
  HeaderInfo = packed record
    ...
    ID: array [1..4] of AnsiChar;                                 { Always "TWIN" }
    Version: array [1..8] of AnsiChar;                               { Version ID }
    ...
    Tag: array [TChunkType] of AnsiString;             { Tag information }
  end;
  ...
  TTwinVQ = object(TObj)
    private
      ...
      procedure SetTagItem(const ID, Data: AnsiString; var Header: HeaderInfo);
...
procedure TTwinVQ.SetTagItem(const ID, Data: AnsiString; var Header: HeaderInfo);
var
  Iterator: TChunkType;
begin
...
begin
    try
      ...
      if Destination = FileName then DeleteFile(PKOLChar(FileName));
      if MoveFile('~', PKOLChar(Destination)) then
    except
