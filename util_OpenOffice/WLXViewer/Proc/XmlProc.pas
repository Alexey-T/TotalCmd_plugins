...
procedure ExtractFB2Images(const fn, TempPath: string);
...
procedure ExtractFB2Images(const fn, TempPath: string);
var
  sContent, paramValue, tagValue, fnImage: string;
begin
  if FReadToString(fn, sContent) then
    while GetXmlTagParamAndValue(sContent, 'binary', 'id', paramValue, tagValue) do
      begin
      fnImage:= TempPath + '\' + paramValue;
      tagValue:= SDecodeBase64(tagValue);
      FWriteString(fnImage, tagValue);
      end;
end;
