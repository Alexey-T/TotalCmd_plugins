...
function UnzipSingle(const fn, dir: string; masks: array of PChar): boolean;
...
begin
  if @Wiz_SingleEntryUnzip=nil then
    begin
    Result:= false;
    MessageBox(0, 'Cannot load unzip32.dll', 'Error', MB_OK or MB_ICONERROR or MB_taskmodal);
    Exit
    end;
  ...
  Result:= Wiz_SingleEntryUnzip(inclNum, incl[1], 0, excl, DCList, UFuncs) = PK_OK;
end;
