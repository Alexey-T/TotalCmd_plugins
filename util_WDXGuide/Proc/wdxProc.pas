...
function WdxList(
  ...
  List: TTntListView; //nil for nField>=0
...
uses
  Windows, SysUtils, Forms, ContPlug, SProc;
  ...
  try
    ...
    repeat
      ...
      try
        ...
        res:= ft_nomorefields;
        Application.MessageBox('Exception in ContentGetSupportedField',
      ...
      for nUnit:= 0 to nUnits-1 do
            ...
            if not fShowText then
              ...
              repeat
                try
                  ...
                  res:= ft_fileerror;
                  Application.MessageBox('Exception in ContentGetValue',
                    PChar(Application.Title), MB_OK or MB_ICONERROR);
                end;
        ...
        if Assigned(List) then
          with List.Items.Add do
            ...
            Caption:= fName;
            SubItems.Add(sUnit);
            SubItems.Add(WdxFieldType(fType));
            SubItems.Add(sVal);
            if fType=ft_fulltext then
    ...
    if @ProcU<>nil then
      try
        ...
        Application.MessageBox('Exception in ContentPluginUnloading',
          PChar(Application.Title), MB_OK or MB_ICONERROR);
      end;
    Sleep(100);
    FreeLibrary(hLib);
