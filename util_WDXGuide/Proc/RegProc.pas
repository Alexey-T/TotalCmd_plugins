// RegProc.pas - registry reading/writing
...
begin
  h:= OpenRegKey(RootKey, SubKey);
  if (RegQueryValueEx(h, Name, nil, @DataType, PByte(DataPtr), @DataSize)<>ERROR_SUCCESS)
    or (DataType<>REG_BINARY)
    then begin DataPtr:= nil; DataSize:= 0 end;
  RegCloseKey(h);
end;
